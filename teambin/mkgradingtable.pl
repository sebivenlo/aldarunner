...
while(<PROC>){
	...
	$filename =~ s/_/\\_/g;
	if ($mod =~ /^SEN1/) {
	    $taskHash{$question}= qq($q & $mod &  $maxpoints &    & $filename\\\\\\hline\n);
...
\\rowcolor{gray} {\\color{white}Task ID} & {\\color{white}Module} & {\\color{white}SEN1} & {\\color{white}JAVA2} &{\\color{white}Source File}\\\\\\hline\n);
foreach my $key (sort keys %taskHash) {
    print qq($taskHash{$key});
}
print qq(\\rowcolor{gray}{\\color{white}Total} &{\\color{white}SEN1}& {\\color{white}$sweightSum} & & {\\color{white}SEN1 approximately $sminutes minutes of work.}\\\\\\hline\n);
print qq(\\rowcolor{gray}{\\color{white}Total} &  {\\color{white}JAVA2}&& {\\color{white}$jweightSum} & {\\color{white}JAVA2 approximately $jminutes minutes of work.}\\\\\\hline\n);
